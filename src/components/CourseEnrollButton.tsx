...
type Props = { slug:string; status:string; canAccess:boolean; pending:boolean; isFree:boolean; priceLabel:string };
export default function CourseEnrollButton({slug,status,canAccess,pending,isFree,priceLabel}:Props){
 ...
 if(!signedIn)return <div className="course-auth-actions"><Link className="course-signup-button" href={`/signup/?course_id=${encodeURIComponent(slug)}&next=/course/${encodeURIComponent(slug)}/`}>Sign up to enrol <span>→</span></Link><small>Already have an account? <Link href={`/login/?next=/course/${encodeURIComponent(slug)}/`}>Sign in here</Link></small></div>;
 ...
 return <>{error&&<p className="course-enrol-error">{error}</p>}<button className={pending?"course-pending-button":"course-buy-button"} onClick={enroll} disabled={busy}>{busy?"Opening secure checkout...":pending?"Payment pending":isFree?"Enrol for free":`Buy now - ${priceLabel}`}<span>→</span></button></>;
}
